...
def main() -> None:
    ...
    for timestep in range(55):
        ...
        w_comparison = np.subtract(u, imposed_w)
        ...
        u = ax['u'].imshow(w_comparison)
        ax['u'].set_yticklabels([])
        ax['u'].set_xticklabels([])
        fig.colorbar(u, ax=ax['u'])
        ax['u'].set_title("W comparison")
        if abs(comparison_mean) < w_std:
            color = 'g'
        else:
            color = 'r'
        fig.text(0.175, 0.3, ("Mean = " + str(comparison_mean)), color=color)
        fig.text(0.175, 0.25, ("Std = " + str(w_std)))

        w = ax['w'].imshow(w)
        ax['w'].set_yticklabels([])
        ax['w'].set_xticklabels([])
        fig.colorbar(u, ax=ax['w'])
        ax['w'].set_title("In plane displacement")
        if abs(u_mean) < u_std:
            color = 'g'
        else:
            color = 'r'
        fig.text(0.45, 0.3, ("Mean = " + str(u_mean)), color=color)
