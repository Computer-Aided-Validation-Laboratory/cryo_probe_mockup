...
def main() -> None:
    # In plane RBM
    base_dir1 = Path.cwd() / "DIC results/In pipe/Out of plane/new_calib"
    base_dir2 = Path.cwd() / "DIC results/Out of pipe/Out of Plane/Calib2/Optimised"
    ...
    for timestep in range(55):
        ...
        w_disps2.append(w)
        w_comp2.append(w_comparison)




    u_disps1 = np.dstack(u_disps1)
    w_comp1 = np.dstack(w_comp1)
    w_disps1 = np.dstack(w_disps1)
    w_comp2 = np.dstack(w_comp2)
    ...
    # W disps
    w_mean1 = np.mean(w_comp1, axis=2)
    w_std1 = np.std(w_disps1, axis=2)
    w_mean2 = np.mean(w_comp2, axis=2)
    ...
    fig.suptitle("Out of plane rigid body motion", size=16)
    fig.text(0.475, 0.9, "Inside pipe", size=14)
    fig.text(0.475, 0.48, "Outside pipe", size=14)

    # plt.show()
    filename = Path.cwd() / "Error_maps/out_of_plane_noisefloor.svg"
